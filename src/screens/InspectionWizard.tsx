...
function AcceptSignStep(props: Props & { data: SalesBrainWorkflowData }) {
  ...
  const request = async () => { if (signatureSendingBlocked(props.inspection, props.signatureRequest)) return; setResult(""); try { await props.onRequestSignature({ customerEmail: props.data.customer.email, customerName, selectedOptionId: selected!.id, message, idempotencyKey }); setResult("Signature request created. The asynchronous SignWell status update will confirm when the invitation is sent.") } catch (error) { setResult(error instanceof Error ? error.message : "The Holloman Signature Service could not create the signature request.") } }
  return <StepContainer icon={<FileSignature size={20} />} title="Accept & Sign" sub="The Holloman Signature Service sends the official invitation, signature fields, and reminders">
    ...
    <FormCard title="Signature Invitation"><ReadOnlyRow label="Customer" value={customerName || "Customer name required"} /><ReadOnlyRow label="Email" value={props.data.customer.email || "Customer email required"} /><label className="py-2 block"><span className="text-xs text-steel font-semibold">Invitation Message</span><textarea value={message} onChange={(event) => setMessage(event.target.value)} rows={4} className="mt-1 w-full border border-surface rounded-xl p-3 text-sm" /></label><button onClick={() => void request()} disabled={props.providerActionLoading || !selected || !customerName || !props.data.customer.email || signatureSendingBlocked(props.inspection, props.signatureRequest)} className="w-full bg-brand-red text-white rounded-xl py-3 font-bold disabled:opacity-40"><FileSignature size={17} className="inline mr-2" />Send for Signature</button></FormCard>
    {props.signatureRequest ? <div className="bg-white rounded-2xl p-4 shadow-sm"><div className="flex items-center justify-between"><strong>Signature Status</strong><span className="uppercase text-sm font-bold text-brand-red">{props.signatureRequest.status.replace("_", " ")}</span></div><div className="text-xs text-steel mt-2">Provider document ID: {props.signatureRequest.providerDocumentId || "Waiting for provider"}</div>{props.signatureRequest.status === "completed" ? <p className="text-sm text-success mt-2">Signed agreement and audit trail saved. The quote is Accepted and PestPac handoff is ready.</p> : null}</div> : null}
    {result ? <div className="rounded-xl bg-surface p-3 text-sm">{result}</div> : null}
    <p className="text-xs text-steel">The quote is not marked Accepted until SignWell's verified Completed status update arrives through the Holloman Signature Service.</p>
  </StepContainer>
}
...
function PestPacHandoffStep(props: Props & { data: SalesBrainWorkflowData }) {
  ...
  if (props.signatureRequest?.status !== "completed") return <StepContainer icon={<Lock size={20} />} title="PestPac Handoff" sub="Available after verified signature completion"><EmptyState title="Waiting for completed signature" detail="The Holloman Signature Service must report Completed before the office handoff opens." /></StepContainer>
  ...
  return <StepContainer icon={<ClipboardList size={20} />} title="PestPac Handoff" sub="Office verifies the permanent operational record">
    ...
    <FormCard title="PestPac Record"><TextRow label="Bill-To Number" value={handoff.billToNumber} onChange={(value) => patch({ billToNumber: value })} /><TextRow label="Location Number" value={handoff.locationNumber} onChange={(value) => patch({ locationNumber: value })} /><TextRow label="Service / Order / Agreement Reference" value={handoff.pestPacReferenceNumber} onChange={(value) => patch({ pestPacReferenceNumber: value })} /><ReadOnlyRow label="Selected Option" value={selected?.name || handoff.selectedOptionName} /><ReadOnlyRow label="One-Time Price" value={selected ? `$${(selected.oneTimePriceCents / 100).toLocaleString()}` : "—"} /><ReadOnlyRow label="Recurring Price" value={selected ? `$${(selected.recurringPriceCents / 100).toLocaleString()}` : "—"} /><ReadOnlyRow label="Provider Document ID" value={handoff.boldSignDocumentId} /><ReadOnlyRow label="Holloman Signature ID" value={handoff.signatureEnvelopeId || "Not recorded"} /><TextRow label="Agreement Date" type="date" value={handoff.agreementDate} onChange={(value) => patch({ agreementDate: value })} /><TextRow label="Signature Date" type="date" value={handoff.signatureDate} onChange={(value) => patch({ signatureDate: value })} /></FormCard>
    <FormCard title="Document References"><ReadOnlyRow label="Signed Agreement" value={handoff.signedAgreementR2Key || "Not saved"} /><ReadOnlyRow label="Audit Trail" value={handoff.auditTrailR2Key || "Not saved"} /><p className="py-2 text-xs text-steel">Upload these to PestPac if its attachment area supports them. Otherwise record the provider document ID and Customer Files reference in PestPac notes.</p></FormCard>
