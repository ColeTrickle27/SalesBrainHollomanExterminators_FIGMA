...
export type QuoteAction = "view" | "download" | "email" | "signature"
  ...
  return (
    <div className="pb-24 px-4 pt-5 max-w-6xl mx-auto">
      <div className="flex items-center justify-between gap-3 mb-4">
        <div>
          <h1 className="font-display text-2xl font-bold text-brand-dark uppercase tracking-wide">
            Quote History
          </h1>
          <p className="text-sm text-steel">
            Saved Sales Brain records from authenticated Ops Brain storage
          </p>
        </div>
        <button
          onClick={onRefresh}
          className="text-sm text-brand-red font-semibold flex items-center gap-1"
        >
      ...
      <div className="grid sm:grid-cols-3 gap-3 mb-4">
        <Metric
          icon={CheckCircle}
          label="Accepted"
          value={`$${((metrics?.acceptedRevenueCents || 0) / 100).toLocaleString()}`}
          sub={`${metrics?.acceptedCount || 0} accepted quote${
            metrics?.acceptedCount === 1 ? "" : "s"
          }`}
        />
        <Metric
          icon={TrendingUp}
          label="Close Rate"
          value={
            metrics?.closeRatePercent == null
              ? "—"
              : `${metrics.closeRatePercent.toFixed(1)}%`
          }
          sub="Accepted / decided · 30 days"
        />
        <Metric
          icon={Percent}
          ...
          sub="Accepted quotes with costing"
        />
      </div>
      ...
      <div className="grid md:grid-cols-[1fr_auto] gap-2 mb-4">
        <div className="relative">
          ...
          <input
            ...
            className="w-full bg-white border border-surface rounded-xl pl-9 pr-3 py-2.5 text-sm focus:outline-none focus:border-brand-red"
            placeholder="Search quotes…"
          />
        </div>
        <div className="flex gap-2 overflow-x-auto">
          {["all", "draft", "sent", "accepted", "declined"].map((value) => (
            <button
              key={value}
              ...
              className={`px-3 py-2 rounded-xl text-xs font-bold border capitalize ${
                status === value
                  ...
                  : "bg-white border-surface text-steel"
              }`}
            >
              {value}
            </button>
      ...
      {error ? (
        <div className="bg-danger-light border border-danger/25 rounded-2xl p-4 flex items-center gap-2 text-sm text-danger">
        ...
        {filtered.map((estimate) => (
          ...
          >
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <span
                      className={`text-xs font-bold px-2 py-0.5 rounded-full capitalize ${statusStyle[estimate.status]}`}
                    >
                      {estimate.status}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center justify-between gap-2 mt-2 text-xs">
                    ...
                    {estimate.totalCents !== null ? (
                      <span className="font-mono font-bold text-brand-dark">
                        ${(estimate.totalCents / 100).toLocaleString()}
                      </span>
                    ) : null}
                  </div>
                </div>
                ...
                <button
                  onClick={() => {
                    if (
                      ...
                      void onDelete(estimate.id).catch(() => undefined)
                  }}
            ...
            <div
              ...
              aria-label={`Quote actions for ${estimate.estimateNumber}`}
            >
              <button
                onClick={() => onAction(estimate.id, "view")}
                className="rounded-lg border border-surface px-2 py-2 text-xs font-bold text-brand-dark hover:bg-surface"
              >
                View PDF
              </button>
              <button
                onClick={() => onAction(estimate.id, "download")}
                className="rounded-lg border border-surface px-2 py-2 text-xs font-bold text-brand-dark hover:bg-surface"
              >
                Download
              </button>
              <button
                onClick={() => onAction(estimate.id, "email")}
                className="rounded-lg border border-surface px-2 py-2 text-xs font-bold text-brand-dark hover:bg-surface"
              >
                Send Email
              </button>
...
function Metric({
  ...
  return (
    <div className="bg-white rounded-2xl p-4 shadow-sm">
      ...
      <div className="font-display text-3xl font-bold text-brand-dark mt-2">
        {value}
      </div>
      <div className="text-xs text-steel mt-1">{sub}</div>
    </div>
  )
}
