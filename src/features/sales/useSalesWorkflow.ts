...
export function useSalesWorkflow() {
  ...
  const importGraphData = async (graphKey: string) => {
    try {
      ...
      updateInspection((previous) => {
        ...
        const existingById = new Map(previous.findings.map((item) => [item.id, item]))
        ...
        const retainedPhotos = previous.photos.filter(
          (photo) => photo.source !== "bugman-graph" || photo.sourceGraphKey !== graphKey,
        )
        return {
          ...previous,
          property: propertyFor(previous, graphKey),
  ...
  const addCustomRecommendation = (description: string, priceCents: number) => {
    ...
    updateInspection((previous) => {
      ...
      return {
        ...
        pricingSnapshot: { currency: "USD", totalCents: priceCents, lineItems: [{ id, label: normalized, amountCents: priceCents }], quotedAt: now },
      }
    })
  }

  const updateCustomRecommendation = (id: string, description: string, priceCents: number) => {
    const normalized = description.trim()
    if (!normalized || !Number.isSafeInteger(priceCents) || priceCents < 0) {
      throw new Error("Enter a custom service description and a valid price.")
    }
    updateInspection((previous) => {
      const updated = previous.recommendations.map((item) => item.id === id && item.isCustom
        ? { ...item, name: normalized, description: normalized, totalCents: priceCents, lineItems: [{ id, label: normalized, amountCents: priceCents }] }
        : item)
      return {
        ...previous,
        recommendations: updated,
        pricingSnapshot: previous.selectedRecommendationId === id
          ? { currency: "USD", totalCents: priceCents, lineItems: [{ id, label: normalized, amountCents: priceCents }], quotedAt: new Date().toISOString() }
  ...
  const removeCustomRecommendation = (id: string) => {
    updateInspection((previous) => ({
      ...previous,
      recommendations: previous.recommendations.filter((item) => item.id !== id || !item.isCustom),
      selectedRecommendationId: previous.selectedRecommendationId === id ? undefined : previous.selectedRecommendationId,
      pricingSnapshot: previous.selectedRecommendationId === id ? undefined : previous.pricingSnapshot,
    }))
  }

  const selectCustomRecommendation = (id: string) => {
    updateInspection((previous) => {
      const recommendation = previous.recommendations.find((item) => item.id === id && item.isCustom)
      if (!recommendation) return previous
      const totalCents = recommendation.totalCents ?? 0
      return {
        ...previous,
        selectedRecommendationId: id,
        pricingSnapshot: { currency: "USD", totalCents, lineItems: recommendation.lineItems.map((item) => ({ id: item.id, label: item.label, amountCents: item.amountCents ?? 0 })), quotedAt: new Date().toISOString() },
      }
    })
  }
  ...
  const buildReport = () => {
    if (!inspection.billTo || !inspection.location || !inspection.pricingSnapshot) {
      setSaveError("Complete the customer and recommended service before building the report.")
      return
    }
    updateInspection((previous) => ({
      ...previous,
  ...
  const updateFindingSummary = (findingId: string, summary: string) => {
    updateInspection((current) => ({
      ...
      findings: current.findings.map((item) => item.id === findingId
        ? { ...item, summary, userEdited: true, updatedAt: new Date().toISOString() }
        : item),
    }))
  }
  ...
  const removePhoto = async (photoId: string) => {
    ...
    updateInspection((current) => ({
      ...
      excludedGraphPhotoIds: photo.source === "bugman-graph"
    ...
    try {
      ...
      setSaveError(error instanceof Error ? error.message : "Unable to remove inspection photo.")
    }
  }
